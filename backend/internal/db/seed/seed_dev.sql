...
insert into name_template (name, type, template, series_show_template, series_season_template, "default")
values ('Main Series Template', 'series', '{{.Title}} - S{{.Season}}E{{.Episode}} - {{clean .EpisodeTitle}} ({{.Year}}) [{{.Quality.Full}}][{{.MediaInfo.AudioCodec}} {{.MediaInfo.AudioChannels}}][{{.MediaInfo.VideoCodec}}]{{if .Release.ReleaseGroup}}-{{.Release.ReleaseGroup}}{{end}}', '{{.Title}} ({{.Year}})', 'Season {{.Season}}', true);
