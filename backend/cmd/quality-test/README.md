...
A CLI tool that compares Snaggle's quality parser against a live Sonarr instance to validate parity.

## Purpose

This tool queries Sonarr's `/api/v3/parse` endpoint with test release titles and compares the quality detection results against Snaggle's internal `quality.ParseQuality()` function. It helps ensure our Go port of Sonarr's quality parsing logic returns identical results.
...
- Detailed breakdown of any mismatches showing both Sonarr's and Snaggle's results
